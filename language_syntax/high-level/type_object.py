...
"""
Когда методы вызываются через экземпляр класса, интерпретатор автоматически передает объект экземпляра в
...
Нахуй нужен? ---> Имеют досутп к атрибутам класса, служит для работы с ними.
Манипулирование данными класса.
Метод не создаётся у объекта экземпляра, что повышает производительность.
"""
a_obj.static_method()
...
print(issubclass(type, object))  # Т.к. множество классов (типов) являются подмножеством множества объектов,
print(issubclass(object, type))  # то логично предположить, что type является подклассом object
