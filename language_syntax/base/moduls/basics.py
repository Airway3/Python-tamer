...
from language_syntax.base.moduls.hi_fag import x, y # Скопировать имя x и y, также видим что выполнился
# модуль hi_fagб а также файлы инит каждого пакета, в них формируется неймспейс каждого пакета
# from .hi_fag import x, y # можно использовать импорт относительно пакета moduls

print(x)
x = 42 # локальной x переопределяем скопированный x из модуля hi_fag
print(x, y)
y[0] = 100 # ИЗМЕНИТЬ y в другом модуле

# Чтобы действительно изменить глобальное имя (а не ток изменяемый объект) в другом файле,
# необходимо использовать инструкцию import:
import hi_fag # Получить имя модуляь, присвоить переменной hi_fag объект модуля
print(hi_fag)
hi_fag.x = 35 # Изменить x в другом модуле
print(x, y, hi_fag.x, hi_fag.sys)

print(hi_fag.__dict__.keys()) # __name__ - имя импортированного файла без .py (в моём случае hi_fag)
